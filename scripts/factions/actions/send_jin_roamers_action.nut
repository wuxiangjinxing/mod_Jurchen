this.send_jin_roamers_action <- this.inherit("scripts/factions/faction_action", {
	m = {},
	function create()
	{
		this.m.ID = "send_jin_roamers_action";
	...
	function onExecute( _faction )
	{
		...
		local party = this.getFaction().spawnEntity(settlement.getTile(), "Jurchen", false, this.Const.World.Spawn.JurchenRoamers, this.Math.min(settlement.getResources(), this.Math.rand(60, 110)));
		party.getSprite("banner").setBrush(settlement.getBanner());
		party.setDescription("A band of eastern army scouting the area.");
